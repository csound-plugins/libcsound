#!/usr/bin/env python3
"""CI test: importing libcsound on linux without csound auto-installs it.

This script must be run on a machine with NO csound installed. Importing
libcsound is then expected to trigger the automatic linux installer, which
downloads the portable csound 7 release, verifies its checksum and installs
it for the current user (~/.local/csound) because stdin is not a terminal.
...
def check_no_csound() -> None:
    problems = []
    userpath = Path.home() / ".local" / "csound"
    if userpath.exists():
        problems.append(f"{userpath} already exists")
    ...
    if ctypes.util.find_library("csound64"):
        problems.append(f"libcsound64 is already findable: {ctypes.util.find_library('csound64')}")
    if problems:
...
def main() -> None:
    ...
    csoundbin = Path.home() / ".local" / "csound" / "csound"
